...
# Zabbix trapper executable
ZABBIX_TRAPPER_PATH = "/usr/bin/zabbix_sender"
...
def use_zabbix_trapper(data_to_send):
    for key, value in data_to_send:
        print(
            'zabbix_trapper -c conf_file -k {0} -o "{1}"'.format(key, str(value)))
        subprocess.call([ZABBIX_TRAPPER_PATH, '-c',
                         ZABBIX_CONF_FILE, '-k', str(key), '-o', 'VALUE'])
...
parser.add_argument("-t", "--trapper", action="store_true")
...
if (args.trapper and
    ...
    parser.error(
        'The --trapper (-t) option cannot be used with any other argument.')
...
# Printing domains in JSON format (only d is defined)
    ...
    for header, zone in zone_list:
        domains.append({"{#DOMAIN}": zone})
    print(json.dumps({'data': domains}))
# Publish general stats using Zabbix Trapper
elif args.trapper:
    data_to_send = []
    for view, header, item, content in general_stats:
        key = "{0}[{1}]".format(header, item)
        value = str(content)
        data_to_send.append((key, value))
    use_zabbix_trapper(data_to_send)
